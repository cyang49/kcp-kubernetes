...
func newKcpMutatingWorkEstimator(countFn KcpWatchCountGetterFunc) WorkEstimatorFunc {
	return newTestKcpMutatingWorkEstimator(countFn, enableMutatingWorkEstimator)
}

func newTestKcpMutatingWorkEstimator(countFn KcpWatchCountGetterFunc, enabled bool) WorkEstimatorFunc {
	estimator := &kcpMutatingWorkEstimator{
		countFn: countFn,
		enabled: enabled,
	}
...
type kcpMutatingWorkEstimator struct {
	countFn KcpWatchCountGetterFunc
	enabled bool
}

func (e *kcpMutatingWorkEstimator) estimate(r *http.Request, flowSchemaName, priorityLevelName string) WorkEstimate {
	...
	cluster, err := apirequest.ValidClusterFrom(r.Context())
	if err != nil {
		return WorkEstimate{
			InitialSeats:      1,
			FinalSeats:        maximumSeats,
	...
	watchCount := e.countFn(cluster.Name.String(), requestInfo)
	// TODO(cyang49): ObserveWatchCount should contain cluster info
